...
static void sam9x60_frac_pll_unprepare(struct clk *hw)
{
	...
	io_clrsetbits32(frac->core.base + AT91_PMC_PLL_CTRL0,
						    ...
						    bool update)
{
	 ...
	 */
	nmul = (rate / parent_rate) * 1;
	nmul += ((rate % parent_rate) * 1) / parent_rate;
	tmprate = (parent_rate / 1) * nmul;
	tmprate += ((parent_rate % 1) * nmul) / 1;
...
static void sam9x60_div_pll_set_div(struct sam9x60_pll_core *core,
				    uint32_t div,
				    bool enable)
{
	vaddr_t regmap = core->base;
...
static TEE_Result
sam9x60_div_pll_set_rate(struct clk *hw, unsigned long rate,
			 unsigned long parent_rate)
{
	struct sam9x60_div *div = hw->priv;
...
static TEE_Result
sam9x60_div_pll_set_rate_chg(struct clk *hw, unsigned long rate,
			     unsigned long parent_rate)
{
	struct sam9x60_div *div = hw->priv;
	struct sam9x60_pll_core *core = &div->core;
...
sam9x60_clk_register_div_pll(struct pmc_data *pmc,
			     ...
			     const struct clk_pll_layout *layout,
			     uint32_t flags, uint32_t safe_div)
{
